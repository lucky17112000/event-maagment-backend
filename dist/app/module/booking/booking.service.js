...
const createBooking = async (payload, user) => {
    ...
    if (idea.status !== "APPROVED") {
        throw new AppError(status.BAD_REQUEST, "Booking can only be made for approved ideas");
    }
    const seatConfig = await prisma.eventSeatConfig.findUnique({
        where: { ideaId },
    });
    if (!seatConfig) {
        throw new AppError(status.BAD_REQUEST, "This event does not require booking");
    }
    const availableSeats = seatConfig.totalSeats - seatConfig.bookedSeats;
    if (seatCount > availableSeats) {
        throw new AppError(status.BAD_REQUEST, `Only ${availableSeats} seats are available`);
    }
    ...
    if (alreadyBooked) {
        throw new AppError(status.BAD_REQUEST, "You have already booked a seat for this idea");
    }
...
const getBookingsByIdea = async (ideaId, user) => {
...
const updateBookingStatus = async (payload, user) => {
    if (user.role !== Role.ADMIN) {
        throw new AppError(status.FORBIDDEN, "Only admin can access this resource");
    }
